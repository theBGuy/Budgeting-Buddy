...
function MonthsGrid({ budget, months, setMonths, remaining, setRemaining }) {
  ...
  function handleChange(e) {
    ...
    const currentMonthBudget =
      Number(e.target.value) < months[month].max
          ...
          <div className="month" key={month}>
            ...
            <div className="month-budget">
              ...
              <input
                id={month}
                type="number"
                value={months[month].budget <= 0 ? "" : months[month].budget}
                min={0}
                max={months[month].max}
                onChange={handleChange}
                disabled={false}
              />
              <AddIcon
                onClick={() => stepUp(month)}
                sx={remaining <= 0 ? inactiveStyle : activeStyle}
              />
            </div>
          </div>
        );
